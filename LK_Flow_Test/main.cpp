...
int flow_example(int argc, const char* argv[]){
    const char* keys =
...
int main(int argc, const char* argv[]){
    ...
    auto getPoints = [](const GpuMat& d_mat, vector<Point2f>& vec){
        vec.resize(d_mat.cols);
        Mat mat(1, d_mat.cols, CV_32FC2, (void*)&vec[0]);
        d_mat.download(mat);
    };

    auto getStatus = [](const GpuMat& d_mat, vector<uchar>& vec){
        vec.resize(d_mat.cols);
        Mat mat(1, d_mat.cols, CV_8UC1, (void*)&vec[0]);
        d_mat.download(mat);
    };
    ...
    Ptr<cuda::CornersDetector> detector = cuda::createGoodFeaturesToTrackDetector(CV_8UC1, 200, 0.01, 20);
    Ptr<cuda::SparsePyrLKOpticalFlow> d_pyrLK_sparse = cuda::SparsePyrLKOpticalFlow::create(Size(21, 21), 3, 30);
    ...
    for(int index=0;index<1000;++index)
    {
        ...
        cv::Mat img_curr;
        cv::cvtColor(img_curr_color, img_curr, COLOR_BGR2GRAY);
        ...
        // goodFeaturesToTrack
        GpuMat img_prev_gpu(img_prev);
        GpuMat d_prevPts;
        detector->detect(img_prev_gpu, d_prevPts);

        ticToc.toc_print_tic("CUDA detect");

        GpuMat img_curr_gpu(img_curr);

        GpuMat d_nextPts;
        GpuMat d_status;
        d_pyrLK_sparse->calc(img_prev_gpu, img_curr_gpu , d_prevPts, d_nextPts, d_status);


        vector<Point2f> prevPts(d_prevPts.cols);
        getPoints(d_prevPts, prevPts);

        vector<Point2f> nextPts(d_nextPts.cols);
        getPoints(d_nextPts, nextPts);

        vector<uchar> status(d_status.cols);
        getStatus(d_status, status);

        reduceVector(nextPts,status);

        ticToc.toc_print_tic("CUDA calc");



        vector<cv::Point_<float>> new_pts;
        cv::goodFeaturesToTrack(img_curr, new_pts, 200, 0.01, 20);

        ticToc.toc_print_tic("CPU detect");

        vector<uchar> status_cpu;
        vector<float> err_cpu;
        vector<cv::Point_<float>> curr_pts;
        cv::calcOpticalFlowPyrLK(img_prev, img_curr, new_pts, curr_pts,status_cpu, err_cpu, cv::Size(21, 21), 3);
        reduceVector(curr_pts,status_cpu);

        ticToc.toc_print_tic("CPU calc");



        cv::Mat img_show=img_curr_color;
        for (const auto & nextPt : nextPts){
            cv::circle(img_show, nextPt, 2, cv::Scalar(255, 0, 255 ), 2);
        }
