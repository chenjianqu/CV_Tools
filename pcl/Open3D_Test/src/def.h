...
inline std::vector<double> split_to_double(const std::string& source, const std::string& delimiters = " "){
    std::vector<std::string> tokens = split(source,delimiters);
    ...
    for(int i=0;i<tokens.size();++i) {
        try{
            ...
            cerr<<fmt::format("In split_to_double(),input:{},\nerror:{}",tokens[i],e.what())<<endl;
            std::terminate();
        }
    }
    return values;
